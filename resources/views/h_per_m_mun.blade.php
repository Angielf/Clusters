<table class="table table-striped">
    ...
    <tbody>
        ...
        @if($user_org->amount_of_months_hours() > 0)
        ...
        <tr>
            ...
            <td>
                @if($user_org->amount_estimated_per_1() > 0)
                    {{$user_org->amount_estimated_per_1() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_real_per_1() > 0)
                    {{$user_org->amount_real_per_1() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_estimated_per_2() > 0)
                    {{$user_org->amount_estimated_per_2() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_real_per_2() > 0)
                    {{$user_org->amount_real_per_2() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_estimated_per_3() > 0)
                    {{$user_org->amount_estimated_per_3() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_real_per_3() > 0)
                    {{$user_org->amount_real_per_3() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_estimated_per_4() > 0)
                    {{$user_org->amount_estimated_per_4() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_real_per_4() > 0)
                    {{$user_org->amount_real_per_4() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_estimated_per_5() > 0)
                    {{$user_org->amount_estimated_per_5() }}
                @endif
            </td>
            <td>
                @if($user_org->amount_real_per_5() > 0)
                    {{$user_org->amount_real_per_5() }}
                @endif
            </td>
        </tr>
        @endif
